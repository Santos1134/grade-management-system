...
-- Sponsors table (must be created before students due to foreign key)
CREATE TABLE public.sponsors (
  ...
  CONSTRAINT section_required_for_junior_grades_sponsor CHECK (
...
CREATE TABLE public.students (
  ...
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  CONSTRAINT section_required_for_junior_grades_student CHECK (
    (grade IN ('7th Grade', '8th Grade') AND section IS NOT NULL) OR
    (grade NOT IN ('7th Grade', '8th Grade') AND section IS NULL)
  )
);
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    VALUES (NEW.student_id, 'New grades have been added for ' || NEW.subject);
  ELSIF TG_OP = 'UPDATE' THEN
    INSERT INTO public.notifications (student_id, message)
    VALUES (NEW.student_id, 'Grades have been updated for ' || NEW.subject);
  END IF;
